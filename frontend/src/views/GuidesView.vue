...
const tutorials = computed(() => guides.tutorials)
const loading = computed(() => guides.loading)
const error = computed(() => guides.error)
...
async function loadTutorials() {
  const fishId = selectedFishIds.value.length ? selectedFishIds.value[0] : null
  const method = fishId ? '' : selectedMethod.value
  await guides.loadTutorials({ method, fishId })
  currentPage.value = 1
}
...
<template>
  <section class="space-y-6">
    <header class="flex flex-col gap-4">
      ...
      <div
          v-if="submittedInfo"
          class="rounded-md border border-emerald-700/50 bg-emerald-700/10 px-4 py-3 text-sm flex items-center justify-between gap-3"
      >
        <span>Poradnik został wysłany do moderacji.</span>
        <button
            type="button"
            class="px-3 py-1.5 text-sm rounded-md border bg-[var(--color-bg)] hover:bg-black/5"
            @click="dismissSubmitted"
        >
          OK
        </button>
      </div>
      ...
      >
        ...
        <div v-else-if="fishSearch.length >= 2" class="text-xs opacity-70">
          Brak wyników.
        </div>

        <div v-if="selectedFishList.length" class="pt-3 border-t border-gray-200">
    ...
    <div v-if="!loading && !error && tutorials.length > 0" class="space-y-4">
      <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        ...
        >
          ...
          <div class="p-4 flex flex-col gap-3">
            ...
            <div v-if="t.methods && t.methods.length" class="flex flex-wrap gap-1 mt-1">
              <span
            ...
            <p v-if="t.snippet" class="text-sm leading-relaxed mt-2">
      ...
      <nav v-if="totalPages > 1" class="flex items-center justify-center gap-2 pt-2">
        <button
            type="button"
